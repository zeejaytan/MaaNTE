...
| R8 | **（实测确认，结论已封闭）** GFN Chrome 网页版窗口模式下，页面自绘头部（标题条，实测 37px，非早期目测的 26px）占据客户区顶部，游戏视频被下移且按比例缩放，所有固定 ROI 识别失败（实测 InWorld 失败 5563 次、SceneManager 连按 ESC 569 次死循环）。后续 PRD 已修正自动缩放的头部高度计算与截图缩放配置（帧几何本身确认修正正确、无失真），但 `InWorld` 等基础设施级识别使用贴近顶部的固定 ROI（`EscMenuButton`/`TasksMenuButton`），+37px 平移仍使其结构性失败；进一步排查子窗口截图方案（`Chrome_RenderWidgetHostHWND` + `PW_RENDERFULLCONTENT`）证实截图技术可行，但 MaaFramework 的窗口发现 API（`MaaToolkitDesktopWindowFindAll`）只枚举顶层窗口、无法声明式命中该子窗口，且无运行时更换控制器目标 hwnd 的 API——在当前 MaaFramework/MXU 能力下无法在 MaaNTE 侧解决。 | GFN Chrome 窗口模式**仍不支持自动化任务**（帧几何虽已修正，但识别仍失败） | 运行前提：16:9 显示器 + F11 全屏（头部消失、视频铺满、帧缩放为干净 1280x720）+ Windows 缩放 100%；文档与 UI 提示中声明，作为唯一可用路径。完整实测过程与排除方案见 [GFN Chrome 网页版窗口自动缩放 PRD](./gfn-chrome-window-resize-prd.md) §5 |
